...
function formatCurrency(input, blur) {
    ...
    if (input_val.indexOf(".") >= 0) {
        ...
        left_side = formatNum(left_side);

        // validate right side
        right_side = formatNum(right_side);
        ...
        if (data.intStatus != 1) {
            ...
            if (data.lstHeader.length > 0) {
                ...
                for (var i = 0; i < data.lstHeader.length; i++) {
                    ...
                    if (header.IsShow) {
                        contents += '<th class="f-xs" align="left" data-title="' + header.Caption + '" ' + (header.Width == 0 ? '' : 'width="' + header.Width +'"') + '>' + header.Caption + '</th>';
                    }
                }
...
function LoadTablePaging(divid, url, page) {
    if ($("#" + divid).length == 0) {
        return;
    }
    var search = null;
    ...
    $.post(url + '&page=' + page + '&pagesize=' + pagesize + (search == null ? '' : '&' + search) + '&session=' + (session == null ? "" : session), function (data) {
        ...
        if (data.intStatus != 1) {
            ...
            if (data.intTotalPage > 1) {
                ...
                for (var i = start; i < data.intCurrentPage ; i ++) {
                    contents += '<li><a href="javascript:LoadTablePaging(\'' + divid + '\',\'' + url + '\',\'' + i + '\');" class="waves-effect"> ' + i + ' </a></li>';
                }
                contents += '<li><a class="waves-effect"> <b>' + data.intCurrentPage + '</b> </a></li>';
                for (var i = data.intCurrentPage + 1; i < data.intCurrentPage + 5 && i <= data.intTotalPage; i++) {
                    contents += '<li><a href="javascript:LoadTablePaging(\'' + divid + '\',\'' + url + '\',\'' + i + '\');" class="waves-effect"> ' + i + ' </a></li>';
