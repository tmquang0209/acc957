...
<select class="form-control" id="Field01" name="Field01"><option value="">Đăng Ký</option>
...
<select class="form-control" id="Field02" name="Field02"><option value="">Pet</option>
...
<select class="form-control" id="Field03" name="Field03"><option value="">Thẻ Vô Cực</option>
<option value="1">Có</option>
<option value="2">Không</option>
</select>
                                            </div>
...
$data = $db->query("SELECT * FROM `nick` WHERE `loainick` = '3' AND `status` = '0' $id $money $field01 $field02 $field03 LIMIT $from,$limit");
}else{
$data = $db->query("SELECT * FROM `nick` WHERE `loainick` = '3' AND `status` = '0' LIMIT $from,$limit");
}
foreach($data as $row){
                                    ...
                                    <div class="attribute_info">
                                        <div class="row">
                                                    <div class="col-xs-6 a_att">
                                                        Đăng ký: <b><?=load_info1($row['info1']);?></b>
                                                    </div>
                                                    <div class="col-xs-6 a_att">
                                                        Pet: <b><?=load_info2($row['info2']);?></b>
                                                    </div>
                                                    <div class="col-xs-6 a_att">
                                                        Thẻ vô cực: <b><?=load_info3($row['info3']);?></b>
                                                    </div>
                                        </div>
                                    </div>
                                    <div class="a-more">
...
<?php } ?>                         
                    ...
                    <div class="dataTables_wrapper no-footer">
                        ...
                        <?php 
$tong = $db->query("SELECT * FROM `nick` WHERE `loainick` = '3' AND `status` = '0'")->rowcount();
if ($tong > $sotin1trang){
    if(isset($_GET)){
if($id != null || $money != null || $field01 != null || $field02 != null || $field03 != null){
